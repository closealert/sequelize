...
module.exports = (function() {
  ...
  DAOFactory.prototype.init = function(daoFactoryManager) {
    ...
    this.idMap = (this.daoFactoryManager.sequelize.options.idMap ? (this.daoFactoryManager.sequelize.options.idMap===true ? new LRUCache(1000) : new LRUCache(this.daoFactoryManager.sequelize.options.idMap)) : false);
  ...
  DAOFactory.prototype.build = function(values, options) {
    ...
    if(!(options.hasOwnProperty('isNewRecord') ? options.isNewRecord : true) && this.idMap) {
      instance = getFromIDMap.call(this,values,options);
      if(instance) {
        return instance;
      }
    }
    ...
    if(!instance.isNewRecord && this.idMap) {
      storeInIDMap.call(this,instance);
    }
  ...
  var getFromIDMap = function(values,options) {
    var key = 'm_' + this.tableName;
    if(this.hasPrimaryKeys) {
      for(var k in this.primaryKeys) {
        key += '_' + values[k];
      }
    }
    else {
  ...
  var storeInIDMap = function(instance) {
    var key = 'm_' + this.tableName;
    if(this.hasPrimaryKeys) {
      for(var k in this.primaryKeys) {
        key += '_' + instance[k];
